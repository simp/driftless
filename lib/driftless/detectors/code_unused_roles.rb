...
module Driftless
  module Detectors
    class CodeUnusedRoles < Callable
      include Exclusions::Classes

      key      'code:unused-roles'
      severity :warning
      quality  :stale
      about 'Role classes not classified by any active node'
      ...
      def call
        ...
        corpus.puppet_classes.values
          ...
          .map do |cls|
            build_finding(
              path:    cls.file,
              message: "role #{cls.fqname.inspect} is not classified by any active node",
