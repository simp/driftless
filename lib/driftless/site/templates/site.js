...
(function (global) {
  ...
  // The repo web link for a path (and line), from the `{path}`/`{line}`
  // template in repo.web; null without a template. Without a line the
  // fragment part of the template is dropped.
  function fileLink(template, path, line) {
    if (!template || !path) { return null; }
    const base = line ? template : template.replace(/#[^#]*$/, '');
    return base.replace('{path}', path).replace('{line}', line ? String(line) : '');
  }
  ...
  function fileRef(path, line, cls) {
    const href = fileLink((data.repo || {}).web, path, line);
  ...
  function locationCell(f) {
    ...
    if (f.line) {
      const href = fileLink((data.repo || {}).web, f.path, f.line);
