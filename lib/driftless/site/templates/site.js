...
(function (global) {
  ...
  const DEFAULTS = {
    view: 'findings',
    sev: [], qual: [], key: '', path: '', group: 'key', summary: '1',
  ...
  // The web link for a path (and line) in the repo.
  //
  // repo: the build data's `repo`; `web` is the control repo's `{path}`/
  //   `{line}` template, `deployments[<prefix>].web` the template for what
  //   the Puppetfile deployed there
  // returns the link from the longest deployment prefix covering the path,
  //   with the path made relative to it, else from `web`; null when the
  //   repo it falls in has no template. Without a line the fragment part
  //   of the template is dropped.
  function fileLink(repo, path, line) {
    if (!repo || !path) { return null; }
    const deployments = repo.deployments || {};
    let prefix = null;
    Object.keys(deployments).forEach(function (p) {
      if (path.indexOf(p + '/') === 0 && (prefix === null || p.length > prefix.length)) { prefix = p; }
    });
    const template = prefix === null ? repo.web : deployments[prefix].web;
    if (!template) { return null; }
    const rel = prefix === null ? path : path.slice(prefix.length + 1);
    const base = line ? template : template.replace(/#[^#]*$/, '');
    return base.replace('{path}', rel).replace('{line}', line ? String(line) : '');
  }
  ...
  // and sort.
  //
  // utilization: the build data's `utilization`, or null
  // state: reads cat, by, match, count, sort, dir. `sort` is `name`, `count`,
  //   or `col:<column>`; a column absent from `columns` sorts as `name`.
  //   Ties fall back to name.
  // returns { columns, rows, total, badCount }: `columns` is the breakdown
  //   (collectors or environments) seen across the rows, one table column
  //   per value; `badCount` when the count expression did not parse
  function utilizationRows(utilization, state) {
    ...
    const col = state.sort.indexOf('col:') === 0 ? columns.indexOf(state.sort.slice(4)) : -1;
    rows.sort(function (a, b) {
      let r = state.sort === 'count' ? a.nodes - b.nodes : col >= 0 ? a.cells[col] - b.cells[col] : 0;
  ...
  const model = {
    ...
    location: location, fileLink: fileLink, filterFindings: filterFindings, summarize: summarize,
  ...
  // A path (and line) as text, or as a link into the repo's web interface
  // when the build data carries a template.
  function fileRef(path, line, cls) {
    const href = fileLink(data.repo, path, line);
    const node = href ? el('a', { class: cls, href: href, target: '_blank', rel: 'noopener' }, [path])
      : el('span', { class: cls }, [path]);
    return node;
  }

  // "path:line" with each part styled as the text writer colors it.
  function locationCell(f) {
    if (!f.path) { return el('td', { class: 'location' }, ['-']); }
    const parts = [fileRef(f.path, f.line, 'path')];
    if (f.line) {
      const href = fileLink(data.repo, f.path, f.line);
      parts.push(':', href ? el('a', { class: 'line', href: href, target: '_blank', rel: 'noopener' }, [String(f.line)])
        : el('span', { class: 'line' }, [String(f.line)]));
    }
    return el('td', { class: 'location' }, parts);
  }
  ...
  //
  // A discrete action (opts.push: a click on a link, checkbox, select, tab,
  // or reset) gets its own history entry so Back undoes it; keystrokes in a
  // text field replace the current entry instead of becoming one step each.
  function update(changes, opts) {
    ...
    if (hash !== global.location.hash && !(hash === '' && global.location.hash === '')) {
      const url = hash || global.location.pathname + global.location.search;
      if (opts && opts.push) { global.history.pushState(null, '', url); } else { global.history.replaceState(null, '', url); }
    }
    if (viewChanged || (opts && opts.full)) { render(); } else { renderResults(); }
  }

  // The filter keys each view owns; what reset and ESC clear.
  const FILTER_KEYS = {
    findings: ['sev', 'qual', 'key', 'path', 'group'],
    hierarchy: [],
    utilization: ['cat', 'by', 'sort', 'dir', 'match', 'count'],
  };

  function resetFilters() {
    const c = {};
    FILTER_KEYS[state.view].forEach(function (k) { c[k] = defaultState()[k]; });
    update(c, { full: true, push: true });
  }
  ...
  function checkboxGroup(label, values, selected, key, styleFn) {
    return el('span', { class: 'group' }, [el('span', null, [label])].concat(values.map(function (v) {
      ...
      input.addEventListener('change', function () {
        ...
        const change = {};
        change[key] = next;
        update(change, { push: true });
      });
      return el('label', null, [input, styleFn ? styleFn(v) : v]);
    })));
  }

  // ref, when given, receives the input element (for keyboard shortcuts).
  function textInput(label, key, placeholder, ref) {
  ...
  // A labelled select bound to one state key.
  //
  // key: the state key the choice writes
  // options: [value, text] pairs
  // opts: optional; passed to update on change, over { push: true }
  // returns the label element
  function select(label, key, options, opts) {
    ...
    sel.addEventListener('change', function () {
      const c = {};
      c[key] = sel.value;
      update(c, Object.assign({ push: true }, opts));
    });
    return el('label', null, [label, sel]);
  }

  // A table header that sorts the table by its column.
  //
  // A click on the active column flips the direction; on any other column,
  // sorts by it ascending.
  //
  // key: the `sort` state value this column sorts by
  // cls: optional extra class for the th
  // returns the th element
  function sortHeader(label, key, cls) {
    const active = state.sort === key;
    const classes = (cls ? cls + ' ' : '') + 'sortable' + (active ? ' sorted' : '');
    return el('th', { class: classes, on: { click: function () {
      update(active ? { dir: state.dir === 'desc' ? 'asc' : 'desc' } : { sort: key, dir: 'asc' }, { push: true, full: true });
    } } }, [label + (active ? (state.dir === 'desc' ? ' ▾' : ' ▴') : '')]);
  }

  function resetButton() {
    return el('button', { type: 'button', title: 'ESC', on: { click: resetFilters } }, ['reset']);
  }
  ...
  // keys: single-letter shortcuts the view answers to when no field has focus.
  function findingsControls(shownEl, keys) {
    const focus = function (input) { return function () { input.focus(); input.select(); }; };
    const controls = el('div', { class: 'controls' }, [
      ...
      textInput('key', 'key', 'substring', function (input) { keys.k = focus(input); }),
      textInput('path', 'path', 'substring', function (input) { keys.p = focus(input); }),
      select('group by', 'group', [['key', 'detector'], ['path', 'path']]),
      resetButton(),
      shownEl,
    ]);
    keys.g = function () { update({ group: state.group === 'path' ? 'key' : 'path' }, { push: true, full: true }); };
    controls.querySelectorAll('label').forEach(function (label, i) {
      const hint = ['k', 'p', 'g'][i - SEVERITIES.length - QUALITIES.length];
      if (hint) { label.setAttribute('title', 'shortcut: ' + hint); }
    });
    return controls;
  }
  ...
  // A collapsible panel — a sidebar on a wide viewport — so the list below
  // stays in view while filtering; its open state rides in the hash.
  function summarySection(findings) {
    const s = summarize(findings);
    const section = el('details', { id: 'summary' }, [el('summary', null, ['Summary (' + s.total + ')'])]);
    if (state.summary !== '0') { section.setAttribute('open', 'open'); }
    section.addEventListener('toggle', function () {
      const next = section.open ? '1' : '0';
      if (next !== state.summary) { update({ summary: next }); }
    });
    if (!s.total) {
    ...
    const rows = s.rows.map(function (r) {
      return el('tr', null, [
        ...
        el('td', { class: 'key' }, [el('a', { href: '#', on: { click: function (e) {
          e.preventDefault();
          update({ key: r.key, group: 'key' }, { push: true });
        } } }, [r.key])]),
      ]);
    });
  ...
  function findingsSection(findings) {
    ...
    groupFindings(findings, state.group).forEach(function (g) {
      ...
      const heading = byPath
        ? el('h3', { class: 'by-path' }, [g.key === '-' ? el('span', { class: 'location' }, ['-']) : fileRef(g.key, null, 'location'),
          el('span', { class: 'count' }, ['(' + plural(g.items.length, 'finding') + ')'])])
        : el('h3', { class: 'by-key' }, [severitySpan(first.severity), qualitySpan(first.quality),
          el('span', { class: 'key' }, [g.key]),
          el('span', { class: 'count' }, ['(' + plural(g.items.length, 'finding') + ')'])]);
      const rows = g.items.map(function (f) {
        const lead = byPath
          ? el('td', { class: 'key' }, [severitySpan(f.severity), ' ', f.key,
            f.line ? el('span', null, [':', el('span', { class: 'line' }, [String(f.line)])]) : null])
          : locationCell(f);
        return el('tr', null, [lead, el('td', { class: 'message' }, [f.message])]);
      });
      groups.push(el('tbody', { class: 'group' }, [
        el('tr', { class: 'group-head' }, [el('th', { colspan: '2' }, [heading])]),
      ].concat(rows)));
    });
    // One table for every group, so the location column is one width
    // throughout and the messages line up — the text writer's tabularize.
    section.appendChild(el('table', { class: 'findings' }, groups));
    return section;
  }
  ...
  function findingsView() {
    ...
    const keys = {};
    return {
      controls: [warningsSection(data.warnings || []), findingsControls(shownEl, keys)],
      keys: keys,
      sidebar: true,
      results: function () {
    ...
    const trs = rows.map(function (r) {
      ...
      const notes = Object.keys(r.notes).sort().map(function (k) {
        return el('div', { class: 'tier-note' }, [el('a', { href: '#', on: { click: function (e) {
          e.preventDefault();
          // Every findings filter is cleared, so a severity or quality left
          // checked earlier cannot hide the linked key's findings.
          const c = { view: 'findings' };
          FILTER_KEYS.findings.forEach(function (name) { c[name] = defaultState()[name]; });
          c.key = k;
          update(c, { push: true });
        } } }, [k]), ': ' + plural(r.notes[k], 'finding')]);
      });
      return el('tr', { class: 'tier' }, [
        el('td', null, [r.name + (r.declared ? '' : ' (not declared)'),
          r.line ? el('div', { class: 'tier-note' }, [fileRef('hiera.yaml', r.line, 'path'), ':', el('span', { class: 'line' }, [String(r.line)])]) : null]),
  ...
  function utilizationView() {
    ...
    const shownEl = el('span', { class: 'shown' });
    // The breakdown columns are sort choices too, so the group-by select
    // rebuilds the controls.
    const columns = utilizationRows(data.utilization, state).columns;
    const controls = el('div', { class: 'controls' }, [
      select('show', 'cat', CATEGORIES.map(function (c) { return [c, c]; })),
      select('group by', 'by', [['', 'none'], ['collector', 'collector'], ['environment', 'environment']], { full: true }),
      select('sort by', 'sort', [['name', 'name'], ['count', 'count']].concat(columns.map(function (c) { return ['col:' + c, c]; }))),
  ...
  function utilizationSections(shownEl) {
    ...
    const head = [sortHeader(state.cat.replace(/s$/, ''), 'name'), sortHeader('nodes', 'count', 'count')]
      .concat(result.columns.map(function (c) { return sortHeader(c, 'col:' + c, 'count'); }));
  ...
  function renderNav() {
    ...
    VIEWS.forEach(function (v) {
      ...
      b.addEventListener('click', function () { update({ view: v }, { push: true }); });
      nav.appendChild(b);
    });
    nav.hidden = false;
  }
  ...
  function renderResults() {
    results.className = !view.sidebar ? '' : (state.summary === '0' ? 'with-summary summary-closed' : 'with-summary');
    while (results.firstChild) { results.removeChild(results.firstChild); }
    view.results().filter(Boolean).forEach(function (s) { results.appendChild(s); });
  }

  // Back/forward (popstate) and a hand-edited hash both restore from the URL.
  function restore() {
    state = parseHash(global.location.hash);
    render();
  }
  global.addEventListener('popstate', restore);
  global.addEventListener('hashchange', restore);

  // ESC clears the view's filters from anywhere; the view's single-letter
  // shortcuts (findings: k, p, g) apply only while no field has focus.
  document.addEventListener('keydown', function (e) {
    ...
    const action = view && view.keys && view.keys[e.key];
