...
(function (global) {
  ...
  function checkboxGroup(label, values, selected, key, styleFn) {
    return el('span', { class: 'group' }, [el('span', null, [label])].concat(values.map(function (v) {
      ...
      input.addEventListener('change', function () {
        const next = values.filter(function (x) { return x === v ? input.checked : selected.indexOf(x) >= 0; });
        const change = {};
        change[key] = next;
        update(change);
      });
  ...
  function findingsControls(all, shown) {
    return el('div', { class: 'controls' }, [
      ...
      el('span', { class: 'shown' }, [shown + ' of ' + plural(all, 'finding')]),
    ]);
  }
  ...
  function findingsView() {
    ...
    return [
      ...
      findingsControls(all.length, shown.length),
      summarySection(shown),
  ...
  function utilizationView() {
    const section = el('section', { id: 'utilization' }, [el('h2', null, ['Utilization'])]);
    if (!data.utilization) {
      section.appendChild(el('p', { class: 'empty' },
    ...
    const controls = el('div', { class: 'controls' }, [
      ...
      el('span', { class: 'shown' }, [result.rows.length + ' of ' + result.total + ' ' + state.cat]),
    ]);
  ...
  function render() {
    ...
    const sections = state.view === 'hierarchy' ? hierarchyView()
      : state.view === 'utilization' ? utilizationView() : findingsView();
    sections.filter(Boolean).forEach(function (s) { app.appendChild(s); });
  }
